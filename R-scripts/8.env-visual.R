...
theme_set(theme_bw())
#input-----------
env <- read.csv("input/env1.csv", row.names = 1
)%>%
  filter(Project == "Parima")
env1<- env%>%
  select(-c(Station, Site, Habitat, Season, Mudstat, Project))
facts <- env%>%
  select(c(Station, Site, Habitat, Season, Mudstat, Project))

for (i in 1:length(facts)){ 
  facts[i] <- factor(facts[,i])
}
str(facts)
str(env)

env$Site.Habitat<- interaction(env$Site, env$Habitat)
env$Season.Habitat<- interaction(env$Habitat, env$Season)
env$Season.Site <- interaction(env$Season, env$Site)
env$factors <- interaction(env$Site, env$Habitat, env$Season)
env$factors <- interaction(env$Station, env$Site, env$Season)
...
ave <- env %>% group_by(Season, Site, Habitat)%>%
  summarise(Temp = mean(Temp, na.rm = T),Sal =  mean(Sal, na.rm = T), DO = mean(DO, na.rm = T),
            pH = mean(pH, na.rm = T), Clay_Silt = mean(Clay_Silt, na.rm = T),
...
std <- env %>% group_by(Season, Site, Habitat) %>%
  summarise(Temp_sd = sd(Temp, na.rm = T),Sal_sd = sd(Sal, na.rm = T),#/ sqrt(sum(!is.na(Sal))), 
            ...
            Clay_Silt_sd = sd(Clay_Silt, na.rm = T),
            Sand_sd = sd(Sand, na.rm = T), TOM_sd = sd(TOM, na.rm = T))

av_sd <- merge(ave[4:length(ave)],std[4:length(std)])
...
box("Site", "Temp")
box("Site", "Sal")
box("Season.Habitat", "pH")
box("Station", "TOM")
box("Season.Habitat", "DO")
box("Station", 'pH')
...
sed_agg <- env %>%
  ...
  summarise(
    ...
    Clay_Silt = mean(Clay_Silt)
  )

# Melt the data to long format for ggplot2
sed_long <- sed_agg %>%
  pivot_longer(cols = c(Sand, Clay_Silt), names_to = "particle", values_to = "percentage")
sed_long$particle<- factor(sed_long$particle, levels= c("Clay_Silt", "Sand"))

# Create the stacked bar plot
# fonts()
sed<- ggplot(sed_long, aes(x = interaction(sed_long$Habitat, sed_long$Site), y = percentage, fill = particle)) +
...
(p4<- plot() +
    ...
    theme( legend.title = element_text(size=10) ,#legend.position = "bottom",
           ...
           axis.title = element_text(size= 12),
           axis.text = element_text(size = 10))+
    annotate("text", x = (load$PC1*3), y = (load$PC2*3), label = load$var))
...
#PCA winer
ggsave("figs/PCA.all.winter.png", plot = pca_win , width = 9, height = 7)
