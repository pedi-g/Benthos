...
sp.trait<- read.csv("input/traits.csv", row.names = 1) 
  #first trait is numinal, 6th and 7th are ordinal, the rest are fuzzy
sp.trait1<- read.csv("input/traits.csv", row.names = 1)
...
hab<- sp.trait%>%select(starts_with("HB"))
  ...
  repT<- sp.trait%>%select(starts_with("RT"))
...
repT<- com.trait%>%select(starts_with("RT"))
