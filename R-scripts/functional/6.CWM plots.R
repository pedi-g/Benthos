...
#boxplot----
#cwm1<- cwm%>% group_by(season, area, habitat)
...
nname<- c('Weight(gr)','Size','Feeding','Movement','Bioturbation',
          'Reporduction','Life span', 'Toughness', 'Form', 'Flexibility')
trait<- c('W','S.','FD','MV','BT','RT','Ls','TN','FR', 'FL')
 ######### non fuzzy traits ##########
...
p<- ggplot(t, aes(x = season.area, y = t[[4]], fill = habitat ))+
...
fdbar<- function(i, season = F, area = F, habitat = F){
